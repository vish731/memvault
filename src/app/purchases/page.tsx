...
export default function Purchases() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const url = account ? `/api/purchases?wallet=${account.address.toString()}` : "/api/purchases";
  ...
  async function handleView(id: string) {
    ...
    try {
      const url = account
        ? `/api/purchases/${id}/recall?wallet=${account.address.toString()}`
        : `/api/purchases/${id}/recall`;
      const res = await fetch(url, { cache: "no-store" });
      const data = await res.json();
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 sm:px-10 py-14">
      ...
      <h1 className="text-4xl font-extrabold tracking-tight mb-4 animate-in delay-1">Your purchases</h1>
