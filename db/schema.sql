...
-- Individual Shelby upload accounts, one per connected wallet. Lets each user's
-- uploads run against their own funded account instead of a single shared one.
-- Private key stored server-side (custodial) since browser wallets can't yet sign
-- Shelby's BlobOwnerAuth challenge directly (wrapped message format mismatch).
